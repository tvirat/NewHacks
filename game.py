...
# Load and scale the slime image
slime_image = pygame.image.load('images/slime.png').convert_alpha()
slime_image = pygame.transform.scale(slime_image, (50, 50))  # Adjust size as needed

# Load and scale the heart image
heart_image = pygame.image.load('images/heart.webp').convert_alpha()
...
async def start_screen():
    ...
    while True:
        ...
        if button_start.collidepoint((mx, my)):
            if click:
                await main()  # Call main game loop as async
        if button_exit.collidepoint((mx, my)):
            if click:
                pygame.quit()
                sys.exit()
        ...
        pygame.display.update()
        clock.tick(FPS)
        await asyncio.sleep(0)  # Add this line for async compatibility
...
async def main():
    global background_x, coins, obstacles, slimes, lives, score
    ...
    while running:
        ...
        if monster_rect.bottom > HEIGHT - 135:
            monster_rect.bottom = HEIGHT - 135
        ...
        if random.randint(0, 100) < 5:  # Adjust probability as needed
            ...
            slime_y = HEIGHT - 185  # Fixed y-position for slime
...
asyncio.run(main())  # Add this line to run the main function
...
async def end_screen(final_score):  # Make it async
    global click
    while True:
        ...
        if button_play_again.collidepoint((mx, my)):
            if click:
                await main()  # Restart the game by calling the async main function
        if button_exit.collidepoint((mx, my)):
            if click:
                pygame.quit()
                sys.exit()
...
asyncio.run(end_screen(score))  # Call end_screen as an async function
